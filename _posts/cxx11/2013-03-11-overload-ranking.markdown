...
overloads so that they don't accidentally get selected. [A previous blog post][enableif]
already explains how to do that, the disadvantage being that you need to have
*non-overlapping conditions* (or *disjoint conditions*). Let's take a silly
example:
...
is more noise than anything, so we should fix that with a few using-aliases:
...
Unfortunately, we can't do anything about the ellipsis, as that's not a type,
just a special parameter form.
...
With the magic call being the obvious:
`print_fizzbuzz<15>⁠(select_overload{});`. [And it works, too.][live] Overload
resolution is now an obedient little dog with a cute collar on. Mission
accomplished, I'd say.

 [live]: https://ideone.com/dlj0EZ

---

*Special thanks go to [Johannes Schaub - litb][litb] for helping me figure out
the last part.*
