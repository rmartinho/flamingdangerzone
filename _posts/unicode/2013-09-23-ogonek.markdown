...
perform some mutation on that storage, you cannot do that in-place&emdash;that
...
That leaves an empty string&emdash;empty strings still hold the
invariants&emdash; and the old storage moved into an external object. That
